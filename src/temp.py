...
def get_result_by_request(id, thread_name, data_index):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            write_csv('result_library_address_temp.csv',
...
def get_result_from_file(id):
    results_array = read_csv('result_library_address_final' + File_Number + '.csv', False)
    found_item = None
...
def combine_rules_files():
    file_name1 = 'rules_mohammad.csv'
    file_name2 = 'rules_nazila.csv'
...
def find_category(result, id):
    ...
    file_name = 'rules_mohammad_nazila.csv'
...
def work_with_data(input):
    ...
    write_csv('pre_final_output_temp.csv', ['custom_fields[pid]', 'category', 'subcategory', 'group'])
    ...
    for t in threads:
        for thread_item in t.resultArray:
            if thread_item != [0, 0, 0, 0]:
                write_csv('pre_final_output_temp.csv', thread_item)
...
def groupify(data, type):
    ...
    raw_data = read_csv('final_output_temp.csv', False)
    d1 = pd.DataFrame(raw_data[1:len(raw_data)], columns=raw_data[0])
    grouped = d1.groupby('subcategory')


def check_image_similarities():
    array = read_csv('final_output2.csv', False)
    count = 0
    array_temp = array
    ...
    for item in array_temp:
        write_csv('final_output_temp.csv', item)
        if pd.isnull(item[Subcategory_Index]) is True:
            counter = counter + 1
    # print(str(len(array_temp)) + " null items" + str(counter) )
...
def merge_results():
    raw_data = read_csv('products_part' + File_Number + '.csv', False)
    classified_data = read_csv('pre_final_output_temp.csv', False)
    ...
    for i in classified_data[0][1:]:
        title_array.append(i)
    write_csv('final_output_temp.csv', title_array)
...
file_name = 'products_part' + File_Number + '.csv'
